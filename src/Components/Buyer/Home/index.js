...
export default function ViewItems() {
    ...
    //Get all Products from Database. Check if the time of any of them has ended. If yes, change status of product. Also update IP of Buyer
    useEffect(() => {
        getClientIp().then((result) => {
            firebase.database().ref("/users/Buyer").child(firebase.auth().currentUser.uid).update({
                lastIp: result
            })
        })
    ...
    const goToProductPage = (card, i) => {
        ...
        history.push(`/buyerdashboard/product`)
    }
