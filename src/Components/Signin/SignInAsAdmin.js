...
export default function SignIn(props) {
  ...
  // Validate if an admin account with the entered email exists and then login  
  const signIn = () => {
    var ref = firebase.database().ref("/users/Admin/");
    ref.orderByChild("email").equalTo(formData.email).once("value").then(function (snapshot) {
      if (snapshot.exists() && Object.values(snapshot.val())[0].accounttype == "Admin") {
        ...
        var allErrors = []
        allErrors.push("An Admin Account With This Email Does Not Exist!")
        setErrors(allErrors)
        setTimeout(() => {
          setErrors([])
        }, 5000);
