...
export default function SignIn(props) {
  ...
  // Validate if a buyer account with the entered email exists and is approved and isn't blocked and then login  
  const signIn = () => {
    var ref = firebase.database().ref("/users/Buyer/");
    ref.orderByChild("email").equalTo(formData.email).once("value").then(function (snapshot) {
      if (snapshot.exists() && Object.values(snapshot.val())[0].accounttype == "Buyer") {
        ...
        var allErrors = []
        allErrors.push("The account with this email no longer exists!")
        setErrors(allErrors)
        setTimeout(() => {
          setErrors([])
        }, 5000);
